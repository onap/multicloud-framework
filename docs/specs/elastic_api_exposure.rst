...
spec. But it can be implemented in other workitem(s) in future. And it should be
noted that this spec doesn't prevent adding extension support. Extension support
will be elaborated in `Definition of API`.
...
If the API doesn't have any southbound VIM request, for example, just a local data
handling, other field, like `handler`, can be added in API definition. `handler` can
be path of a python module, which will eventually handle the Multicloud API.
