...
Problem Description
===================
The currently ONAP supports only VM based cloud infrastructure for VNF.
...
Basically all the information in kubeconfig file will be registered.
...
This API is build on top of proxy api.
...
Kubernetes: CSAR
----------------
NOTE: the use of CSAR is temporally work around for Beijing release to avoid
...
* SO calls Multicloud proxy API and passes the entire BluePrint(as CSAR) to
  ...
  POST https://msb.onap.org:80/api/multicloud-kubernetes/v0/clusters/<cloud-id>/proxy/csar
* k8s plugin andles the CSAR accordingly and talks to k8s api server to
  deploy containerized VNF
  POST <k8s api server>://api/v1/namespaces/{namespace}/pods
  to create pods. then <pod id> is obtained
* DELETE https://msb.onap.org:80/api/multicloud-kubernetes/v0/clusters/<cloud-id>/proxy/api/v1/namespaces/{namespace}/pods/<pod id>
  to destroy pod
...
multicloud
----------
new k8s plugin will be introduced. more details are discussed in this
documentation.
...
The eventual work flow looks like as follows::
            ...
            | +------------+-----+ |   requests to multicoud for instanciation.
            +--------------|-------+
                           |
                           |
            +--------------|---------+
